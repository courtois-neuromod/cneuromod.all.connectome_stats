...
`invoke run-smoke` passes, `fetch` retrieves real data, `run-connectomes` computes real per-session, per-network connectomes, and `run-group-stats` computes the two headline analyses plus a domain-restricted robustness check on claim 2 (`analysis/group_stats.py`) into `output_data/group_stats/*.tsv`, plotted by the six real montage panels in `notebooks/figure_connectomes.ipynb` — see "Domain-restricted cross-context figures" below. `run-motion-strata` computes a second robustness-tier check — QC (motion) dependence — into `output_data/motion_strata/*.tsv`, plotted standalone by `notebooks/figure_motion.ipynb` — see "Motion stratification" below.

`fetch-cneuromod`, `fetch-timeseries`, `fetch-parcel-labels`, `run-connectomes`, `run-group-stats`, `run-motion-strata`, `run-figure-layout`, `run-notebooks`, `compose-figure`, `verify` and every `clean-*` task are real.
...
**Honest framing, carried into the figure caption (`motion_note.txt`)**: this tests whether *relative* motion differences inside an already low-motion cohort move connectome similarity. A null is informative about these data; it is not evidence about high-motion data in general.

**`low_motion` vs. `qc_covered`, not vs. `gated`.** `analysis/group_stats.py`'s `GATES` includes `qc_covered` (gated and `fd_mean` present) and `low_motion` (`qc_covered` and `motion_stratum == "low"`, `"cell"` split) alongside `all`/`gated`, delegated to `analysis.motion_strata` from `_gate_mask_for` rather than reimplemented. The meaningful contrast for the headline tables is `low_motion` vs. `qc_covered`: `low_motion` necessarily drops mario/harrypotter (no `fd_mean` coverage), so comparing it against `gated` would confound a motion effect with that change in dataset composition. Verified: claim 2's four-bin ordering (within-subj/within-task > within-subj/between-task > between-subj/within-task > between-subj/between-task) holds in all 9 networks under both `qc_covered` and `low_motion`.
...
**Testing:** Two baseline checks, and they cover different failures. `invoke run-smoke` is the behavioural one: does the pipeline run end to end and produce something. `invoke verify` is the structural one: do the code, config, data and docs still describe the same project. Run both before committing; neither substitutes for the other. Add unit tests in a tests directory, using the project's chosen test framework, when a function contains non-trivial logic, has edge cases the smoke test won't catch, or is shared across multiple steps. Unit tests are optional for simple glue/orchestration code but encouraged for any pure transformation or computation logic in `analysis/`. This project uses `pytest`; tests live in `tests/` — `analysis/parcel_networks.py`, `timeseries_reader.py`, `connectome_estimators.py`, `qc_join.py`, `connectome_store.py`, `similarity.py`, `friends_seasons.py`, `group_stats.py`, `domain_titles.py` and `motion_strata.py` each have real coverage now, all offline against synthetic fixtures.
