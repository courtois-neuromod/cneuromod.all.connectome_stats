...
This project reads parcelled BOLD timeseries. As of **2026-09-05**, 19 `{dataset}/timeseries` submodules are registered in `cneuromod.all`: `floc`, `movie10`, `friends`, `things`, `hcptrt`, `harrypotter`, `mario`, `mario3`, `mariostars`, `petit-prince`, `retinotopy`, `shinobi`, `gamepad`, `langlocalizer`, `multfs`, `mutemusic`, `narratives`, `ood`, `triplets`. The seven added on 2026-09-04 (`gamepad` onward) are registered mountpoints with no content fetched here yet, so every `run-*` step warns and skips them. `fetch-timeseries` discovers them via `_list_datasets` rather than reading that list.
...
- **No session in the gated population reaches `fd_mean > 0.3`.** Over the 659 gated sessions, max is 0.248, median 0.121, 99th pct 0.222 — CNeuroMod adults, averaged over a >=30 min session, are a uniformly low-motion cohort. So the split is **relative** (median split within a group), not an absolute mm threshold.
- **`fd_mean` covers only 304 of 659 gated sessions.** `mario` (empty QC table upstream) and `harrypotter` (blank `session` column upstream) have zero coverage, so the domain stratification (movies/videogames/stories) cannot carry this analysis — it runs **pooled over all FD-covered datasets** instead: `friends`, `movie10`, `hcptrt`, `shinobi`, `petit-prince`, `multfs`, `mutemusic`, `narratives`.
...
**Strata**: `motion_stratum in {low, high}` = below/above the median `fd_mean` within each **(subject, dataset)** cell (`assign_motion_strata`, split `"cell"`), cells with fewer than 4 covered sessions dropped (276 of 304 sessions survive). Motion is strongly subject- and dataset-specific (median `fd_mean` ranges 0.085-0.187 across subjects; sub-02 alone is 9 high/0 low on shinobi vs. 3 high/14 low on friends), so splitting within subject alone would leave "high motion" partly meaning "shinobi" — the (subject, dataset) cell makes the stratum orthogonal to both by construction. The plain **within-subject** median split (304 sessions) is computed and written alongside it as a secondary `split` column in every table (`"cell"` vs. `"subject"`), not a second code path.

`fd_mean` vs. `tsnr` is **r = -0.68** — the motion stratum is substantially a tSNR stratum, and `motion_balance.tsv` reports both together, never as independent axes. `fd_mean` vs. usable duration is **r = -0.070** and the strata come out duration-balanced (a few percent apart), so the duration confound that forced the 1800 s gate does not bite here.
...
**Strata** mirror the motion check exactly: median split within each (subject, dataset) cell (`split="cell"`, primary, 276 of 304 sessions after dropping cells with fewer than 4 covered sessions), with the plain within-subject split (304 sessions) alongside as `split="subject"`.

**Duration is a mild confound, and it points the wrong way to explain the result.** `tsnr` vs. usable duration is r = 0.08 at the session level, and `tsnr_balance.tsv` shows the strata are duration-balanced within the within-task bin (high-high vs. low-low pair-min duration 2748 s vs. 2704 s under `raw`, 1.02x; 2712 s vs. 2732 s under `fd_residual`). The between-task bin is *not* balanced: 2722 s vs. 2361 s under `raw`, a 1.15x gap, shrinking to 1.06x under `fd_residual`. Since similarity rises with duration, that gap would push high-high pairs *up* relative to low-low — the opposite of the observed negative `observed_diff`, so it cannot be what produces the effect. Report it; do not claim this axis is duration-clean.
